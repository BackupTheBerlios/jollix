#!/bin/sh
# mkxorgconf.sh
# based on mkxf86config by knoppix
...
# Create mouse link and insert a mouse default type into xorg
...
if [ ! -f /etc/xorg.conf -a ! -f /etc/X11/xorg.conf ]; then 
rm -f /etc/xorg.conf /etc/X11/xorg.conf 2>/dev/null
...
# xorg-x11 hack
XORGSERVER="XOrg-X11"
echo -n " using ${YELLOW}${XORGSERVER:-generic VESA}"
